...
export {
    ...
    type Info: record {
        ...
        orig_payload_bytes: count    &log &optional; 
        resp_payload_bytes: count    &log &optional;
        ...
        # HTTP Features
        http_requests:      count    &log &default=0;
        http_error_codes:   count    &log &default=0;
...
# --- HTTP Hooks ---
event http_request(c: connection, method: string, original_URI: string, unparsed_URI: string, version: string) {
    if ( c?$ml_record ) c$ml_record$http_requests += 1;
}

event http_reply(c: connection, version: string, code: count, reason: string) {
    if ( c?$ml_record && code >= 400 ) c$ml_record$http_error_codes += 1;
}
...
event ftp_reply(c: connection, code: count, msg: string, cont_resp: bool) {
    if ( ! c?$ml_record ) return;
    if ( code == 530 ) c$ml_record$ftp_failed_auths += 1;
    else if ( code == 230 ) c$ml_record$ftp_success_auths += 1;
}
...
event ssl_client_hello(c: connection, version: count, record_version: count, possible_ts: time, client_random: string, session_id: string, ciphers: index_vec, comp_methods: index_vec)
{
    ...
    if ( version == SSL_v20 ) c$ml_record$tls_version = "SSLv2";
    else if ( version == SSL_v30 ) c$ml_record$tls_version = "SSLv3";
    else if ( version == TLS_v10 ) c$ml_record$tls_version = "TLSv1.0";
    else if ( version == TLS_v11 ) c$ml_record$tls_version = "TLSv1.1";
    else if ( version == TLS_v12 ) c$ml_record$tls_version = "TLSv1.2";
    else if ( version == TLS_v13 ) c$ml_record$tls_version = "TLSv1.3";
    else c$ml_record$tls_version = "Unknown";
}
...
event connection_state_remove(c: connection) &priority=-10 {
    ...
    if ( c?$orig ) {
        c$ml_record$orig_payload_bytes = c$orig$size;
        c$ml_record$orig_pkts = c$orig$num_pkts;
    }
    if ( c?$resp ) {
        c$ml_record$resp_payload_bytes = c$resp$size;
    ...
    if ( c?$ssl ) {
        ...
        if ( c$ssl?$server_name ) c$ml_record$sni_domain = c$ssl$server_name;
    }
